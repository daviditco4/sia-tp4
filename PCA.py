...
# Añadir los valores de PCA1 a la columna correspondiente en el DataFrame
df['PCA1'] = X_pca[:, 0]

# Ordenar el DataFrame por los valores de PCA1
df_sorted = df.sort_values(by='PCA1', ascending=True)

# Graficar el biplot (solo PCA1)
plt.figure(figsize=(10, 6))

# Puntos de los países proyectados en PCA1
plt.barh(df_sorted['Country'], df_sorted['PCA1'], color='cyan')

# Añadir etiquetas de los valores a las barras
for i, value in enumerate(df_sorted['PCA1']):
    plt.text(value, i, f'{value:.2f}', color='black', va='center')

# Configurar el gráfico
plt.xlabel('Países')
plt.ylabel('Valor de PCA 1')
plt.title('Barplot de PCA1 por Países')
