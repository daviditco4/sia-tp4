...
class MultilayerPerceptron:
    ...
    def forward_propagation(self, x, weights=None):
        ...
        activations = [x] * (len(self.weights) + 1)  # Store activations for each layer
        excitations = [None] * len(self.weights)  # Store weighted sums for each layer (before activation)
